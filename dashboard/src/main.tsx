...
type PlanOffer = { id: string; country_code: string; country_name: string; isp: string; name: string; download_mbps: number; upload_mbps: number; price_amount?: number; currency_code?: string; price_php?: number; price_period?: string; category?: string; note?: string; source_url?: string; verified_at?: string; custom?: boolean; advertised_speed_basis?: "up_to" | "average" | "typical" };
...
const countryFlag = (code = "") => {
	...
	return String.fromCodePoint(...[...cc].map(letter => 0x1F1A5 + letter.charCodeAt(0)));
};
...
function PlanModal({ catalog, current, busy, onSave, onClose }: { catalog: PlanOffer[]; current?: PlanOffer; busy: boolean; onSave: (selection: PlanSelection) => void; onClose: () => void }) {
  ...
  const price = amount && currency ? `${new Intl.NumberFormat(undefined, { style: "currency", currency, maximumFractionDigits: Number.isInteger(amount) ? 0 : 2 }).format(amount)} / ${selected?.price_period || "month"}` : "";
  return <div class="modal-backdrop"><section class="modal settings" role="dialog" aria-modal="true" aria-labelledby="plan-title">
    ...
    <label class="plan-field"><span>Country</span><select aria-label="Country" value={countryCode} onChange={event => pickCountry((event.target as HTMLSelectElement).value)}>{countries.map(country => <option value={country.code}>{countryFlag(country.code) + " " + country.name}</option>)}</select></label>
...
function App() {
  ...
  const confidenceLabel = latest ? `${latest.confidence_score}/100` : "No test";
	...
	const account = status.account || { available: false, connected: false, pairing: false };
  ...
  return <main class="shell">
    ...
    <footer><span>FiberPulse {status.version}</span><span>Private by default · Data stored locally</span></footer>
        ...
        <div class="setting-row"><div><b>FiberPulse account</b><span>{account.connected ? `${account.email} · ${account.plan === "pro" ? "Pro" : "Free"}${account.last_sync_at ? ` · synced ${date(account.last_sync_at)}` : ""}` : account.pairing ? "Finish signing in in your browser" : "Use the same history on macOS, Windows and the web"}</span>{account.last_error && <small class="account-setting-error">{account.last_error}</small>}</div>{account.connected ? <div class="setting-actions"><button class="mini-button" disabled={busy} onClick={() => void run("account-sync")}>Sync now</button><button class="mini-button danger" disabled={busy} onClick={() => void run("account-disconnect")}>Disconnect</button></div> : <button class="mini-button" disabled={busy || account.pairing || !account.available} onClick={() => void run("account-connect")}>{account.pairing ? "Waiting…" : "Connect"}</button>}</div>
